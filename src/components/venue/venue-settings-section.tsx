...
import type { VenueEmployee, VenuePermissionSection } from "@/data/types";
...
const PERMISSION_SECTIONS: { id: VenuePermissionSection; label: string }[] = [
  ...
  { id: "payments", label: "Оплаты и начисления" },
  { id: "documents", label: "Документы" },
];

const ALL_PERMISSIONS = PERMISSION_SECTIONS.map((section) => section.id);

const DEFAULT_INVITE_PERMISSIONS: VenuePermissionSection[] = [
  "dashboard",
  "bookings",
  "events",
  "documents",
...
const EMPLOYEE_STATUS_LABELS: Record<VenueEmployee["status"], string> = {
  active: "Активен",
...
export function VenueSettingsSection({ venueId = "venue-1", showToast }: Props) {
  const {
    venueEmployees,
    addVenueEmployee,
    updateVenueEmployee,
  ...
  const [email, setEmail] = useState("venue@example.ru");
  const [inviteEmail, setInviteEmail] = useState("");
  const [inviteFullName, setInviteFullName] = useState("");
  const [invitePhone, setInvitePhone] = useState("");
  const [invitePermissions, setInvitePermissions] = useState<VenuePermissionSection[]>([
  ...
  const employees = useMemo(
    ...
    [venueEmployees, venueId]
  );

  const currentAdmin = employees.find((employee) => employee.isAdmin);

  const toggleInvitePermission = (section: VenuePermissionSection) => {
    setInvitePermissions((prev) =>
      prev.includes(section) ? prev.filter((item) => item !== section) : [...prev, section]
    );
  };

  const toggleEditPermission = (section: VenuePermissionSection) => {
  ...
  const handleInvite = () => {
    ...
    const employeeId = `vemp-${Date.now()}`;
    const inviteLink = `${window.location.origin}/register?invite=${employeeId}&venue=${venueId}`;
  ...
  const openPermissionsModal = (employee: VenueEmployee) => {
  ...
  const savePermissions = () => {
    ...
    updateVenueEmployee(editingEmployee.id, { permissions: editPermissions });
    setEditingEmployee(null);
    showToast("Права доступа обновлены");
  };

  const handleDelegateAdmin = (employee: VenueEmployee) => {
    delegateVenueAdmin(venueId, employee.id);
    showToast(`Права администратора переданы: ${employee.fullName}`);
  };

  const handleRemoveEmployee = (employee: VenueEmployee) => {
    if (employee.isAdmin) {
      showToast("Нельзя удалить администратора кабинета", "error");
      return;
    }
    removeVenueEmployee(employee.id);
        ...
        {employees.map((employee) => {
          const inviteLink =
            employee.status === "invited"
              ? `${typeof window !== "undefined" ? window.location.origin : ""}/register?invite=${employee.id}&venue=${venueId}`
              : null;
