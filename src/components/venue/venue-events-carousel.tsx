...
export function VenueEventsCarousel({ venueId = "venue-1" }: Props) {
  ...
  const venuePhotos = SEED_VENUE_PROFILE_MEDIA.filter(
    ...
    return SEED_EVENTS.filter((event) => event.venueId === venueId)
      .map((event) => {
        ...
        const eventNotifications = notifications.filter(
          (item) => item.eventId === event.id && item.audience === "venue" && !item.read
        );

        return { event, meta, halls, eventNotifications };
      })
      .sort((a, b) => a.event.startDate.localeCompare(b.event.startDate));
  }, [venueId, venueEventMeta, notifications]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-sm font-medium">Мероприятия на площадке</p>
          <p className="text-sm text-gray-600 mt-1">
            Карусель привязана к календарю — в {getMonthLabel(activeMonth).toLowerCase()}{" "}
            {monthEvents.length} мероприятий
