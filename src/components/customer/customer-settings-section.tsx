...
import type { CustomerEmployee, CustomerPermissionSection } from "@/data/types";
import { usePrototypeStore } from "@/lib/store";
import { formatDate } from "@/lib/utils/formatters";

const PERMISSION_SECTIONS: { id: CustomerPermissionSection; label: string }[] = [
  { id: "dashboard", label: "Дашборд" },
  { id: "profile", label: "Профиль компании" },
  { id: "legal", label: "Юридические и платёжные данные" },
  { id: "edo", label: "ЭДО и документооборот" },
...
const DEFAULT_INVITE_PERMISSIONS: CustomerPermissionSection[] = [
  "dashboard",
  "responses",
  "active-projects",
  "documents",
...
export function CustomerSettingsSection({
  ...
  const {
    ...
    removeCustomerEmployee,
    delegateCustomerAdmin,
  } = usePrototypeStore();

  const [email, setEmail] = useState("customer@example.ru");
  const [inviteEmail, setInviteEmail] = useState("");
  const [inviteFullName, setInviteFullName] = useState("");
  const [invitePhone, setInvitePhone] = useState("");
  const [invitePermissions, setInvitePermissions] = useState<CustomerPermissionSection[]>([
    ...DEFAULT_INVITE_PERMISSIONS,
  ]);
  const [editingEmployee, setEditingEmployee] = useState<CustomerEmployee | null>(null);
  const [editPermissions, setEditPermissions] = useState<CustomerPermissionSection[]>([]);
  const [lastInviteLink, setLastInviteLink] = useState<string | null>(null);
  const [emailNotifications, setEmailNotifications] = useState(true);
  const [pushNotifications, setPushNotifications] = useState(true);

  const employees = useMemo(
    () => customerEmployees.filter((employee) => employee.customerId === customerId),
    [customerEmployees, customerId],
  );

  const currentAdmin = employees.find((employee) => employee.isAdmin);

  const toggleInvitePermission = (section: CustomerPermissionSection) => {
    setInvitePermissions((prev) =>
      prev.includes(section) ? prev.filter((item) => item !== section) : [...prev, section],
    );
  };

  const toggleEditPermission = (section: CustomerPermissionSection) => {
  ...
  const handleInvite = () => {
    ...
    const employeeId = `cemp-${Date.now()}`;
    const inviteLink = `${window.location.origin}/register?invite=${employeeId}&customer=${customerId}`;
  ...
  const openPermissionsModal = (employee: CustomerEmployee) => {
  ...
  const savePermissions = () => {
    ...
    updateCustomerEmployee(editingEmployee.id, { permissions: editPermissions });
    setEditingEmployee(null);
    showToast("Права доступа обновлены");
  };

  const handleDelegateAdmin = (employee: CustomerEmployee) => {
    delegateCustomerAdmin(customerId, employee.id);
    showToast(`Права администратора переданы: ${employee.fullName}`);
  };

  const handleRemoveEmployee = (employee: CustomerEmployee) => {
    if (employee.isAdmin) {
      showToast("Нельзя удалить администратора кабинета", "error");
      return;
    }
    removeCustomerEmployee(employee.id);
        ...
        {employees.map((employee) => {
          const inviteLink =
            employee.status === "invited"
              ? `${typeof window !== "undefined" ? window.location.origin : ""}/register?invite=${employee.id}&customer=${customerId}`
              : null;
