...
export function CustomerCheckoutSection({ cartHref }: Props) {
  ...
  const resolvedCartHref = cartHref ?? getCartHref(user?.role);
  ...
  const handleSubmit = async () => {
    ...
    grouped.forEach((group, index) => {
      ...
      const title = group.lines.map((line) => line.title).join(", ").slice(0, 80);
      ...
      const deal: Deal = {
        ...
        documents: [],
        stages: group.lines.map((line, stageIndex) => ({
          id: `st-${dealId}-${stageIndex}`,
          title: line.title,
  ...
  return (
    <>
      <p className="text-sm text-gray-600 mb-4">
        Каждый исполнитель получит отдельный заказ (безопасная сделка). Это позволяет независимо
